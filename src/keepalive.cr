...
def start_keepalive(session : SSH2::Session, interval : Int32, debug : Bool) : Nil
  ...
  spawn do
    loop do
      ...
      begin
        ...
        STDERR.puts "Keepalive sent (next in #{seconds_to_next}s)" if debug && ret == 0
      rescue ex
        STDERR.puts "Keepalive failed: #{ex.message}" if debug
        break
      end
    end
  end
