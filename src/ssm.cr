...
module SSHClient
  def self.fetch_ssm_key(path : String, region : String,
    ...
    client = if access_key_id && secret_key
               creds = if session_token
                         Awscr::SSM::SimpleCredentials.new(access_key_id, secret_key, session_token)
                       else
                         Awscr::SSM::SimpleCredentials.new(access_key_id, secret_key)
                       end
