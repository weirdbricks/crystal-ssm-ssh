...
#
# Limitation: SSH Host pattern negation (e.g. "Host !badhost") is not
# supported. Only simple wildcard matching via File.match? is used.
# ---------------------------------------------------------------------------
...
module SSHClient
  ...
  private def self.expand_tilde(val : String) : String
    if val.starts_with?("~/")
      Path.home.join(val[2..]).to_s
    elsif val == "~"
      Path.home.to_s
    else
      val
  ...
  def self.parse_ssh_config(target_host : String) : SshConfigEntry
    ...
    File.each_line(config) do |raw|
      ...
      when "host"
        ...
        when "port"
          entry.port ||= val.to_i?
        when "identityfile"
          entry.identity_files << expand_tilde(val)
        when "userknownhostsfile"
          entry.known_hosts_file ||= expand_tilde(val)
        when "identitiesonly"
